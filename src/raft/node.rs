...
impl<C: RaftClient> Rafter<C> {
    ...
    fn start_election_timed_task(&self) {
        ...
        tokio::spawn(async move {
            ...
            inner.lock().force_refresh_election_instant();
            loop {
                let (next_election_instant, refresh) = inner.lock().refresh_election_instant();
...
impl<C: RaftClient> Raft for Rafter<C> {
    ...
    async fn request_vote(
        ...
        // TODO: Lock optimize
        Ok(Response::new(
            self.inner.lock().request_vote(request.get_ref()),
        ))
    }
    ...
    async fn append_entries(
        ...
        Ok(Response::new(
            self.inner.lock().append_entries(request.get_ref()),
        ))
    }
}
...
mod test {
    ...
    impl<C: RaftClient> Service for RequestVoteService<C> {
        async fn handle(&self, data: &[u8]) -> Result<Vec<u8>> {
            ...
            let response = self.0.inner.lock().request_vote(&request);
    ...
    impl<C: RaftClient> Service for AppendEntriesService<C> {
        async fn handle(&self, data: &[u8]) -> Result<Vec<u8>> {
            ...
            let response = self.0.inner.lock().append_entries(&request);
            Ok(response.encode_to_vec())
        }
    }
