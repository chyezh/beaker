    ...
    fn start_loop(&mut self, mut rx: mpsc::UnboundedReceiver<Event>) {
        ...
        tokio::spawn(async move {
            ...
            while let Some(event) = rx.recv().await {
                ...
                match event {
                    Event::Election => perform_election(rf).await,
                    Event::Heartbeat => perform_heartbeat(rf).await,
...
// Process a election
async fn perform_election<C: RaftClient>(rafter: Arc<Mutex<RafterInner<C>>>) {
    let prepare_result = rafter.lock().prepare_election();
    ...
    tokio::spawn(async move {
        ...
        while let Some(Ok(vote_reply)) = rx.recv().await {
            if vote_reply.term == current_term && vote_reply.vote_granted {
                ...
                rafter.lock().set_as_follower_in_new_term(vote_reply.term);
                return;
            }
        }
        ...
        if voted > voted_threshold && rafter.lock().set_as_leader_in_this_term(current_term) {
            perform_heartbeat(rafter).await;
        }
    });
}

// Process the heartbeat
async fn perform_heartbeat<C: RaftClient>(rafter: Arc<Mutex<RafterInner<C>>>) {
    let prepare_result = rafter.lock().prepare_heartbeat();
    if prepare_result.is_none() {
        return;
    }
